...
def load_mesh_from_ply(
    ...
    try:
        ...
        if convex_hull:
            ...
            # mesh.show()
        if clean_mesh:
            # Common mesh preprocessing/fixes using trimesh utilities
...
# -----------------------------
# 5) Pooling to a fixed-length vector
# -----------------------------
...
def visualize_descriptor_values(descriptors: np.ndarray, shape_indices=[0, 1]):
    ...
    if D % 2 != 0:
        raise ValueError(f'Descriptor dimension {D} should be even (mean + variance)')

    n_times = D // 2

    fig, axes = plt.subplots(len(shape_indices), 1, figsize=(12, 4*len(shape_indices)))
    ...
    for i, idx in enumerate(shape_indices):
        ...
        for j, val in enumerate(mean_part):
            axes[i].text(x[j] - 0.2, val, f'{val:.2f}', ha='center', va='bottom')
...
if __name__ == '__main__':
    """
    Example workflows for computing scale-invariant HKS descriptors.

    Two approaches are shown:
      A) Reference-based: Compute one time grid, reuse for all shapes
      B) Per-shape: Compute fresh time grid for each shape
    """

    # ========== APPROACH A: Reference-based (more consistent) ==========
    #
    # Step 1: Choose a reference shape and compute its time grid
    # verts_ref, faces_ref = load_mesh_from_ply('reference.ply')
    # L_ref, M_ref = build_laplacian_from_mesh(verts_ref, faces_ref)
    # evals_ref, evecs_ref = lap_eigendecomp(L_ref, M_ref, n_eigs=128)
    #
    # # Create one shared time grid from reference
    # times_shared = make_time_grid(
    #     evals_ref,
    #     M_diag=M_ref.diagonal(),
    #     n_times=16
    # )
    #
    # # Step 2: Process each shape using the shared time grid
    # descriptors = []
    # for mesh_file in ['shape1.ply', 'shape2.ply', 'shape3.ply']:
    #     verts, faces = load_mesh_from_ply(mesh_file)
    #     L, M = build_laplacian_from_mesh(verts, faces)
    #     evals, evecs = lap_eigendecomp(L, M, n_eigs=128)
    #
    #     H = hks_from_eigendecomp(
    #         evals, evecs, times_shared, M_diag=M.diagonal()
    #     )
    #
    #     descriptor = pool_hks(H, M_diag=M.diagonal())
    #     descriptors.append(descriptor)
    #
    # descriptors = np.array(descriptors)  # shape (N, 32) for n_times=16

    # ========== APPROACH B: Per-shape (simpler) ==========
    #
    # Process each shape with its own adaptive time grid
    try:
        descriptors = []
        mesh_files = [
            r'C:\Users\EFESTWIN\source\repos\csc\component_geometry\hks_test\0aad9436-ead8-4651-81a1-8b435012d799.ply',
            r'C:\Users\EFESTWIN\source\repos\csc\component_geometry\hks_test\0aad9436-ead8-4651-81a1-8b435012d799_b.ply',
            r'C:\Users\EFESTWIN\source\repos\csc\component_geometry\hks_test\0b8c24c4-54d5-41b2-9eb0-43acdc34dfc1.ply',
            r'C:\Users\EFESTWIN\source\repos\csc\component_geometry\hks_test\0dd38d21-87ea-4c1d-a0b8-7245b45cd633.ply',
            r'C:\Users\EFESTWIN\source\repos\csc\component_geometry\hks_test\1c3a2fdc-6522-43ab-900c-8f196804cd8d.ply',
        ]

        # NOTE: n_times * 2 = output descriptor dimension
        # Fast (32D descriptor)
        # n_eigs = 64
        # n_times = 16

        # Balanced (64D descriptor) -> STANDARD
        # n_eigs = 64
        # n_times = 32

        # Detailed (128D descriptor)
        n_eigs = 128
        n_times = 64

        for mesh_file in mesh_files:
            print(f'Processing mesh: {mesh_file}')
            verts, faces = load_mesh_from_ply(
                mesh_file,
                clean_mesh=False,
                convex_hull=True
            )
            print(f'    Loaded mesh: {verts.shape}, {faces.shape}')
            print('    Building Laplacian...')
            L, M = build_laplacian_from_mesh(verts, faces)
            print('    Eigen decomposition...')
            evals, evecs = lap_eigendecomp(L, M, n_eigs=n_eigs)
            print(f'    Eigen decomposition shape: {evals.shape}, {evecs.shape}')
            # Each shape computes its own time grid
            print('    Building time grid...')
            times = make_time_grid(
                evals,
                M_diag=M.diagonal(),
                n_times=n_times
            )
            print(f'    Time grid shape: {times.shape}')
            print('    Computing HKS...')
            H = hks_from_eigendecomp(
                evals, evecs, times, M_diag=M.diagonal()
            )
            print(f'    HKS shape: {H.shape}')
            print('    Pooling...')
            descriptor = pool_hks(H, M_diag=M.diagonal())
        ...
        # shape (N, 32) for n_times=16
        descriptors = np.array(descriptors)
        visualize_descriptor_values(descriptors, shape_indices=[0, 3])
        # All descriptors are scale-invariant and comparable
        print('This should be ~0:')
        dist = np.linalg.norm(descriptors[0] - descriptors[1])
        print(f'Distance between identical test shapes: {dist}')

        print('This should not be be ~0:')
        dist = np.linalg.norm(descriptors[0] - descriptors[2])
        print(f'Distance between different test shapes: {dist}')
        dist = np.linalg.norm(descriptors[0] - descriptors[3])
        print(f'Distance between different test shapes: {dist}')

        # check cosine similarity between descriptors
        print('This should be close to 1:')
        cos_sim = np.dot(descriptors[0], descriptors[1]) / (np.linalg.norm(descriptors[0]) * np.linalg.norm(descriptors[1]))
        print(f'Cosine similarity between identical test shapes: {cos_sim}')
        print('This should not be close to 1:')
        cos_sim = np.dot(descriptors[0], descriptors[2]) / (np.linalg.norm(descriptors[0]) * np.linalg.norm(descriptors[2]))
        print(f'Cosine similarity between different test shapes: {cos_sim}')
        cos_sim = np.dot(descriptors[0], descriptors[3]) / (np.linalg.norm(descriptors[0]) * np.linalg.norm(descriptors[3]))
        print(f'Cosine similarity between different test shapes: {cos_sim}')
