...
ghenv.Component.Description = (  # NOQA
    ...
    'mesh files under pending_identity_assets/{identity_id}/meshes/<i>/ and '
    'point cloud PLY files under .../point_clouds/<i>.ply.'
)
...
class CSC_CreateComponentIdentity(Grasshopper.Kernel.GH_ScriptInstance):
    ...
    def _stage_point_cloud_ply(
        ...
        if os.path.exists(ply_path):
            self._addWarning(
                f'Point cloud PLY already exists for identity {identity_id} '
                f'primitive {primitive_index}; skipping overwrite.'
            )
            return {}
    ...
    def process_point_cloud_geometry(
        ...
        staged = self._stage_point_cloud_ply(
            identity_id, 0, cloud, staging_threshold)
    ...
    def process_multiple_point_clouds_geometry(
        ...
        for index, cloud in enumerate(clouds):
            if cloud is None:
                continue
            staged = self._stage_point_cloud_ply(
                identity_id, index, cloud, staging_threshold)
        ...
        try:
            ...
            if len(Geometry) == 1:
                ...
                if Type == 'rubble':
                    if not isinstance(
                            single_geometry,
                            (Rhino.Geometry.Mesh, Rhino.Geometry.PointCloud)):
                        msg = (
                            'The "rubble" type expects a Mesh or PointCloud '
