...
ghenv.Component.Name = 'FetchDetailedGeometry'  # NOQA
ghenv.Component.NickName = 'FetchDetailedGeometry'  # NOQA
ghenv.Component.Category = 'DDU_CSC'  # NOQA
ghenv.Component.SubCategory = '2 Catalog Interface'  # NOQA
ghenv.Component.Description = (  # NOQA
    'Fetches the detailed (high fidelity) snapshot geometry as binary PLY '
    ...
    '- A raw snapshot_id\n\n'
    'Falls back to reduced PLY, then to the inline snapshot geometry '
    '(primitive meshes), when no detailed PLY is available. Point clouds '
    'prefer the full PLY, then the inline preview.'
)


class CSC_FetchDetailedGeometry(Grasshopper.Kernel.GH_ScriptInstance):
    """
    ...
    # Resolution preference chain (detailed, then reduced; inline otherwise)
    resolution_chain = ['detailed', 'reduced']
    # Point clouds: full PLY first, then the inline preview.
    fetch_point_cloud_ply = True
    ...
    def BeforeRunScript(self):
        ...
        self.OutputParams[0+i].Description = (
            'Fetched detailed geometry as Rhino meshes and point clouds '
            '(one object per snapshot primitive)'
        )
        self.OutputParams[1+i].Description = (
            'Per-primitive source: detailed/reduced (mesh PLY), detailed '
            '(point-cloud PLY), or primitive (inline fallback)'
        )
    ...
    def RunScript(self, Input):
        ...
        try:
            ...
            self.Component.Message = (
                f'Fetching detailed geometry for snapshot {snapshot_id}...'
            )
            ...
            pc_count = sum(
                ...
                if kind == 'point_cloud')
            self.Component.Message = (
                f'Fetched detailed geometry ({mesh_count} mesh(es), '
