...
ghenv.Component.Description = (  # NOQA
    ...
    '(primitive meshes), when no detailed PLY is available.'
)
...
class CSC_FetchDetailedGeometry(Grasshopper.Kernel.GH_ScriptInstance):
    ...
    def BeforeRunScript(self):
        ...
        self.OutputParams[0+i].Description = (
            'Fetched detailed geometry as Rhino.Geometry.Mesh objects '
            '(one per snapshot mesh primitive)'
        )
        self.OutputParams[1+i].Description = (
            'Per-mesh source: detailed/reduced (PLY) or primitive '
            '(inline fallback)'
        )
