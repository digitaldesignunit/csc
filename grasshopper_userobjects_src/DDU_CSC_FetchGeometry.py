...
class CSC_FetchGeometry(Grasshopper.Kernel.GH_ScriptInstance):
    ...
    def fetch_geometry_from_api(self, auth_core, component_id, detailed=False):
        ...
        try:
            if detailed:
                ...
                response = auth_core.authorized_get(f'/components/{component_id}/geometry_detailed')
                if response.status_code == 200:
                    ...
                    self._addRemark(f'Detailed geometry not available for {component_id}, trying reduced...')
                    return self.fetch_geometry_from_api(auth_core, component_id, detailed=False)
                else:
                    self._addWarning(f'Failed to fetch detailed geometry: {response.status_code}')
                    return None
            else:
                # Try to fetch reduced geometry first
                response = auth_core.authorized_get(f'/components/{component_id}/geometry_reduced')
                if response.status_code == 200:
                    return {
                        'type': 'reduced',
                        'data': response.text,
                    ...
                    self._addRemark(f'Reduced geometry not available for {component_id}, using primitive...')
                    return None
                else:
                    self._addWarning(f'Failed to fetch reduced geometry: {response.status_code}')
                    return None
                ...
                if parts[0] == 'v':  # vertex
                    ...
                    if len(parts) >= 4:
                        ...
                        for i in range(1, len(parts)):
                            ...
                            if vertex_part:
                                vertex_index = int(vertex_part) - 1  # OBJ is 1-indexed
                                if 0 <= vertex_index < len(vertices):
                                    face_vertices.append(vertex_index)
                        if len(face_vertices) >= 3:
                            faces.append(face_vertices)
            if not vertices or not faces:
                self._addWarning('No valid vertices or faces found in OBJ data')
                return None
            # Create mesh
            mesh = rg.Mesh()
            # Add vertices
    ...
    def fetch_primitive_geometry(self, auth_core, component_id):
        ...
        """
        try:
            response = auth_core.authorized_get(f'/components/{component_id}/geometry')
            if response.status_code == 200:
                ...
                if 'geometry' in comp_data:
                    ...
                    self._addWarning(f'No geometry data found in component {component_id}')
                    return None
            else:
                self._addWarning(f'Failed to fetch component geometry: {response.status_code}')
                return None
    ...
    def primitive_extrusion(self, extrusion_data: dict) -> Rhino.Geometry.Extrusion:
    ...
    def RunScript(self, Input, Detailed: bool):
        ...
        self.InputParams[1].Description = 'True for detailed geometry, False for reduced'
        self.OutputParams[0].Description = 'Fetched geometry as Rhino.Geometry.GeometryBase'
        self.OutputParams[1].Description = 'Geometry type: detailed, reduced, or primitive'
        ...
        try:
            ...
            self.Component.Message = f'Fetching geometry for component {component_id}...'

            # Try to fetch geometry from API first
            geometry_data = self.fetch_geometry_from_api(auth_core, component_id, Detailed)

            # If API geometry not available, fall back to primitive
            if not geometry_data:
                self._addRemark(f'Falling back to primitive geometry for {component_id}')
                geometry_data = self.fetch_primitive_geometry(auth_core, component_id)
            ...
            # Convert geometry data to mesh and add to outputs
            ...
            self.Component.Message = (
                f'Successfully fetched {geometry_data["type"]} geometry for {component_id}'
            )

            self._addRemark(
                f'Fetched {geometry_data["type"]} geometry for component {component_id}'
            )
