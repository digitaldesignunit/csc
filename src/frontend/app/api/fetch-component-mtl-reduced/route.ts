...
/**
 * Fetch the component's reduced material (.mtl) from the backend.
 * Retries once if receiving a 401 Unauthorized.
 */
const fetch_component_material_reduced = async (
  component_id: string,
  retried: boolean = false
): Promise<string> => {
  // Example: /components/<component_id>/material_reduced
  const endpoint_url = `https://api.ddu.uber.space/components/${component_id}/material_reduced`
  ...
  const mtlFileData = await fetch(endpoint_url, {
    ...
    .then(async (response) => {
      console.log(timestamp_string() + `: Get Reduced Material Response Status: ${response.status}`)
      // Check for 401 → attempt a single retry
      if (response.status === 401 && !retried) {
        console.log(timestamp_string() + ': Response Unauthorized! Attempting Retry...')
        return await fetch_component_material_reduced(component_id, true)
      }
    ...
    .catch(async (err) => {
      if (!retried) {
        console.log(timestamp_string() + ': Get Reduced Material Response Rejected! Retrying...')
        console.log('Error:', err)
        return await fetch_component_material_reduced(component_id, true)
      } else {
        console.log(timestamp_string() + ': Get Reduced Material 2nd Response Rejected! Aborting...')
        console.log('Error:', err)
        return ''
      }
    })
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const mtlFileData = await fetch_component_material_reduced(component_id, false)
    ...
    console.error(timestamp_string() + ': Fetch reduced Material Route Failed:', err)
    return NextResponse.json(
      { error: 'Fetch reduced Material Failed!' },
      { status: 500 }
    )
  }
}
