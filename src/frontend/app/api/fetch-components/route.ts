...
const fetch_components = async (
  ...
  const endpoint_url = `https://api.ddu.uber.space/components?page=${page_num}&size=${page_size}&sortkey=${sortkey}&comptype=${comptype}&material=${material}`
  ...
  let components: Array<ComponentData> = await fetch(
    ...
    if (response.status == 401 && !retried) {
      ...
      return await fetch_components(page_num, page_size, sortkey, comptype, material, true)
    }
...
export async function GET(request: NextRequest) {
  ...
  try {
    const components = await fetch_components(page_num, page_size, sortkey, comptype, material, false)
